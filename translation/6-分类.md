...
这是绝对需要的，因为如果分类的名字可能在扩展的类或者其他分类里面已经使用了，会导致不可预计的后果。实际上，最后被实现的那个分类方法会被实际被调用。
...
你想要确认你没有用你的分类方法覆盖其他实现的话，可以设置一个环境变量 OBJC_PRINT_REPLACED_METHODS 为 YES，这样那些被取代的方法名字会打印到 Console 中。现在 LLVM 5.1  不会为此发出任何警告和错误提示，所以自己小心不要在分类中重载方法。
...
我们的经验是，创建一组分类对以后的重构十分有帮助。一个类的接口增加的时候，可能意味着你的类做了太多事情，违背了类的单一功能原则。 之前创造的方法分组可以用来更好地进行不同功能的表示并且把类打破在更多自我包含的组成部分里。
