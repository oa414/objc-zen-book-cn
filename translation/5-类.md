...
Class clusters 在 Apple 的Framework 中广泛使用：一些明显的例子比如  `NSNumber` 可以返回不同哦给你的子类，取决于数字类型如何提供  (Integer, Float, etc...) 或者 `NSArray` 返回不同的最优存储策略的子类。
...
一个经典的例子是如果有一个一样的 UIViewController 子类，为 iPad 和 iPhone 写，但是在不同的设备上有不同的行为。
