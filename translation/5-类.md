...
你应该总是把你的类加上 **三** 个字母的前缀（两个字母的为 Apple 的类保留）。这个难看的的实践是为了减少我们最喜欢的语言没有命名空间的影响。

一些开发者在 Model 对象中不使用这个实践（我们保留了这个实践，特别是 Core Data 对象），我们建议严格按照这个约定。即使在 Core Data对象中。因为你可能会把你的 Managed Object Model 用其他 MOM 管理，可能来自第三方库。你可能注意到了，这本书的类（不仅仅）前缀都是`ZOC`。
...
这里同样有一个实践你可能遵照来命名你的类的：当你创建一个子类的时候，你应该把它的特别的名字放在类前缀和超类的名字中间。举个例子：如果你有一个 `ZOCNetworkClient` 类，子类的名字会是`ZOCTwitterNetworkClient` (注意 "Twitter" 在 "ZOC" 和 "NetworkClient" 之间); 按照这个约定， 一个`UIViewController` 的子类会是 `ZOCTimelineViewController`.
...
推荐在有 `dealloc` 方法的代码里面，把它放在实现的顶部。（直接在  `@synthesize` 以及 `@dynamic` 之后） 并且 `init` 应该放在 `dealloc`  之后。在多个初始化方法的类中 designated initializer 应该放在第一个，因为这样逻辑性更好，并且 secondary initializer可以在之后。
...
在ARC的陌生下，你几乎不需要实现 dealloc 方法，但是关于dealloc的基本远离和 init 方法很像。你应该在方法中进行强调，同城在 dealloc 中你需要取消执行在 init方法做的事情。
...
理解为什么设置`self`为 `[super init]` 的返回值，以及中间发生了什么，这是一个十分有趣的话题。
...
让我们后退一步：我们曾经打了类似`[[NSObject alloc] init]` 的表达式，that the difference between `alloc` and `init` fades away. 。一个 Objective-C 的特性叫 *两步创建* 。 这意味着申请分配内存和初始化是两个分离的操作。
...
designated初始化方法是提供所有的参数，secondary初始化方法是一个或多个，提供一个或者更多的默认参数来调用designated初始化方法的初始化方法。
...
一个类应该又切只有一个指定的初始化方法么，其他的初始化方法应该调用这个特定的（虽然这个情况有一个例外）
...
正如之前的描述么，第二个初始化是一种方便提供默认值、行为到 特定初始化函数的 方法。
...
当你要实现相等性的时候记住这个约定：你需要同事实现`isEqual` and the `hash`方法。如果两个对象是被`isEqual`认为相等的，它们的 `hash` 方法需要返回一样的值。但是如果  `hash` 返回一样的值，并不能确保他们相等。
