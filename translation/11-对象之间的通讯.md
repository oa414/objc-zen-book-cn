...
不是很麻烦的事情。但是。。。当 block 被 self 在一个属性retain（就像下面的例子）然而 对象（self）retain 了block呢
...
这就是有名的 retain cycle 并且通常应该避免。我们收到 cLANG的警告：
...
你可能会想，首先，这是避免 retain cycle  警告的一个技巧。然而不是，这个到 self 的强引用在 block 的执行时间被创建。当在 block 使用 self 的时候在 block 的定义时间。这样就 retain 了对象

-------------------
[Apple documentation][blocks_caveat1] reports that "For non-trivial cycles, however, you should use" this approach: 
[Apple documentation][blocks_caveat1] 表示为了避免循环引用，你应该这样：
...
首先，我觉得这个例子看起来是错误的。self  如何被 delloc 和被 nil 在 block 之外，如果 block 本身被  completionHandler 属性里面 retain 了呢。 completionHandler 属性。completionHandler 属性可以被声明为  `assign` or `unsafe_unretained`  来允许对象来 dealooc 在 block 被传递之后。

我不能理解这样做的理由，如果其他对象需要这个对象（self），blick被传递的时候应该 retain 对象然而 block 应该不被作为 属性赋值。这种情况下不应该用 `__weak`/`__strong` 
...
更多的，Apple的 "trivial block" 是什么呢。我们的理解是一个 trivial block 是一个不被传送的 block ，它在一个良好定义和控制的作用域里面允许，weak 修饰只是为了避免循环引用。
...
在 Kazuki Sakamoto & Tomohiko Furumoto) 讨论的 [a][blocks_caveat2] [lot][blocks_caveat3] [of][blocks_caveat4] [online][blocks_caveat5] [references][blocks_caveat6], books ([Effective Objective-C 2.0][blocks_caveat14] by [Matt Galloway][blocks_caveat16] and [Pro Multithreading and Memory Management for iOS and OS X][blocks_caveat15] ，大多数开发者始终没有捋清楚概念。
...
在 block 内用强引用的优点是优先的时候的鲁棒性。看上面的三个例子，在 block 执行的时候
...
如果 block  被属性 retain，self  和 block之间会有一个循环引用并且 它们不会再被释放。如果 block 被传送并且被其他的copy了，self 在每一个 copy 里面被 retain
...
没有引用虚幻并且block是否被retain或者是一个 属性都没关系。如果block 被传送或者copy了，执行的时候，weakSelf可能会变成nil
...
block 的执行可以抢占并且不同的后来的对weakSelf的调用可以导致不同的值(比如，weakSelf可能编程nil在 一个特定的执行)
...
没有 retain 圈并且，同时，不论是 block 在retain或者是一个属性的时候。如果block被传送并且被复制了，执行的时候，weak Sefl可能被nil，当强应用被复制并且不会变成nil的时候，我们确保对象是ratin在block调用的完整周期里面，如果抢占发生了随后的对 strongSefl的执行会继续并且会产生一样的值，因为对象被retain了。如果strongSelf的执行到nil，通常执行已经返回了，从block不能执行属性
...
在一个 ARC 的环境中，编译器会警告一个错误，如果尝试用  `->`符号来表示，错误很简单：
...
* **Case 3**: it is related to concurrent executions. When asynchronous services are involved, the blocks that are passed to them can be executed at a later period and there is no certainty about the existence of the self object. 和并发执行有关。当异步的服务涉及的时候，block 可以被在之后执行并且不会有关于 self 存在的问题。
