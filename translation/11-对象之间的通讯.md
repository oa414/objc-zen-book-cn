# Inter-Object Communication 对象之间的通讯
...
每一个不是玩票的软件都是基于对象的，同时为了完成复杂的任务,需要进行对象间的通讯。本章将会深入讨论一些设计观念来解释如何设计并且实践伟大的架构。
...
-------------------
* 如果 `objects` 非 nil 的, error` 必须也是 nil 的
...
此外，作为同步方法，一些 Apple 的　API 在成功状态下向 error 参数（如果非 NULL) 写入了垃圾值（garbage values），所以检查 error 的值可能出现问题。
...
当使用代码块和异步分发的时候，要注意避免循环引用。 总是对变量使用 `weak` 引用会导致循环引用。 此外，把持有 blocks 的属性设置为 nil (比如 `self.completionBlock = nil`) 是一个好的实践。它会打破 blocks 捕获的作用域带来的引用循环。
...
这个情况下 block 没有 retain 对象并且对象 retuan 了block 在 属性里面。所以我们要确保我们安全的访问 self。 最糟糕的是，可能是 nil 的。问题是：如果和让 self  在 block 里面被销毁。
...
默认情况下，委托对象需要实现 protocol 的方法。可以用`@required` 和  `@optional` 关键字来标记方法是必要的还是可选的。
