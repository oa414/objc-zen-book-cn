...
这个特性非常适合组织小块的代码，通常是设置一个类。他让读者一个重要的线索并且减少干扰，能让读者聚焦于关键的变量和函数中。此外，这个技术有一个优点，所有的变量都在代码块中，也就是只在代码块的区域中有效，这意味着可以减少对其他作用域的命名污染。
...
上面的标记能显示分离和组织代码。一个优点是你可以用  cmd+Click 来快速跳转到定义ide地方。
但是小心即使这样 paragma mark 是一门手艺，它不是让你类里面方法数量增加的一个理由：有太多方法说明你的类做了太多事情而且是一个重构的机会了。
...
当大多数 iOS 开发者没有做很多编译器选项的时候，一些选项是对控制严格检查（或者不检查）你的代码或者错误的。有时候，你想要用 pragma 直接产生一个异常，零时打断编译器的行为。
...
当你使用ARC的时候，编译器帮你插入了内存管理相关的调用。这是一个情况，可能产生一些恼火的事情。比如你使用  `NSSelectorFromString`  来动态地产生一个 selector 调用的时候，ARC不知道这个方法是哪个并且不知道应该用那种内存管理方法，你会被提示 `performSelector may cause a leak because its selector is unknown`.
...
这对表明你一个定义但是没有使用的变量很有用。大多数情况下，你希望移除这些引用来提高性能（小小的），但是有时候你希望保留它们。为什么？或许它们以后有用或者有些特性只是暂时移除。无论如何，一个好的方法来消除这些警告是把它们残忍地用相关语句注释，使用 `#pragma unused()`:
...
短文档适用于单行的文件，包括注释斜杠。它用来剪短的函数，特别是（但不仅仅是）非 public 的 API：
