...
`alloc` 方法将返回一个有效的未初始化的对象实例。对这个实例发送的消息会被转义成 `objc_msgSend()` 函数的调用，参数 `self` 指向的是 `alloc` 所返回的指针；这样 `self` 在所有方法的作用域内都能够被访问。

按照惯例，为了完成两步创建，新申请的实例第一个被调用的方法是 `init` 方法。注意，`NSObject` 在实现 `init` 时，只是简单的返回了 `self`。
